...
# rattler-build doesn't set MESON_ARGS by default
MESON_ARGS="${MESON_ARGS:-}"

# setuptools >= 81 vendors a distutils without msvccompiler.py, which
# gobject-introspection's g-ir-scanner still imports; use the stdlib copy
...
# solid (non-multiarch) libdir
if [[ $target_platform == linux-* ]] ; then
  export MESON_ARGS="${MESON_ARGS} --libdir=lib"
fi
