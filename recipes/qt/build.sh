...
if [[ $(uname) == "Linux" ]]; then
    ...
    # mamba create --yes --prefix "${SRC_DIR}/openssl_hack" --channel local \
 ...
 # Shorten the log to a little further on travis and to make the log files smaller
  ...
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qtwebchannel | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qtsvg | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qtsensors | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qtcanvas3d | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qtconnectivity | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qttools | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qtlocation | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} module-qt3d | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
  CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make -j${MAKE_JOBS} | sed "s/^g++.*-o/g++ [...] -o/" | sed "s/-DQT.* //" || exit 1
...
make -j${MAKE_JOBS} || exit 1
CPATH=$PREFIX/include:$BUILD_PREFIX/src/core/api make install || exit 1
popd
...
# fix BUILD_PREFIX err, replace with PREFIX
find $PREFIX -name "*.pc" -type f | xargs  sed -i "s#$BUILD_PREFIX#$PREFIX#g"
find $PREFIX -name "*.pri" -type f | xargs  sed -i "s#$BUILD_PREFIX#$PREFIX#g"
